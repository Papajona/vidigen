...
async function removeBackground(){
 ...
 return <div className="app">
  <header className="topbar"><div className="brand"><div className="brandMark">V</div><span>Vidigen</span><b>V12</b></div><div className="projectTitle">AI Production Studio<small>{clips.length} clips • {captions.length} captions • {profile.successCount} learned preferences</small></div><div className="topActions"><span className={`enginePill ${online?'online':''}`}><i/> {online?'Gateway online':'Offline'}</span><button className="ghost" title="New project" onClick={newProject}>New</button><button className="ghost" title="Undo (Ctrl/⌘ + Z)" onClick={undo} disabled={!undoStack.length}>Undo</button><button className="ghost" title="Redo (Ctrl/⌘ + Shift + Z)" onClick={redo} disabled={!redoStack.length}>Redo</button><button className="ghost" title="Open Creative Brain" onClick={()=>setShowBrain(true)}>Brain</button><button className="export" onClick={()=>setShowExport(true)}>Export</button><div className="accountMenuWrap"><button className={`avatar accountAvatar ${accountMenuOpen?'active':''}`} onClick={()=>setAccountMenuOpen(v=>!v)} aria-label="Open account menu" aria-expanded={accountMenuOpen}>JA</button>{accountMenuOpen&&<div className="accountMenu" role="menu"><div className="accountMenuHead"><b>{token?'Account signed in':'Account'}</b><small>{token?'Your session is active':'Sign in to use your account'}</small></div>{token&&<><button onClick={()=>{setAccountMenuOpen(false);setShowSettings(true)}}>⚙ Account & settings</button><button onClick={()=>{setAccountMenuOpen(false);setNav('Billing');setMobileInspectorOpen(true)}}>◉ Credits & billing</button><button className="accountSignOut" onClick={()=>{setAccountMenuOpen(false);signOut()}}>↪ Sign out</button></>}{!token&&<button className="accountSignIn" onClick={()=>{setAccountMenuOpen(false);setShowAuth(true)}}>Sign in / Create account</button>}</div>}</div></div></header>
  {!token&&<button className="authGateButton" type="button" onClick={()=>setShowAuth(true)} aria-label="Sign in or create your Vidigen account">Sign in / Create account</button>}
