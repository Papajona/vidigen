 ...
 return <div className="app">
  ...
  <div className="railUtilities">
   ...
   <main className="center"><div className="previewHeader"><div><b>{mode}</b><span>{ratio} • {duration}</span></div><div className="previewActions"><button onClick={()=>{if(video.current)video.current.currentTime=Math.max(0,video.current.currentTime-1)}}>−1s</button><button onClick={()=>video.current?.play()}>Play</button><button onClick={()=>video.current?.pause()}>Pause</button><button onClick={()=>{if(video.current)video.current.currentTime+=1}}>+1s</button></div></div>
    <div className="stage">{current?.src?<div className={`videoFrame ratio-${ratio.replace(':','-').replace('.','-')}`} style={{filter}}><video ref={video} src={current.src} controls={false} playsInline onTimeUpdate={()=>{}}/><button className="bigPlay" aria-label="Play preview" onClick={()=>video.current?.paused?video.current?.play():video.current?.pause()}>▶</button>{captions[0]&&<div className={`captionOverlay ${captionStyle.toLowerCase()}`}>{captions[0].text}</div>}{activeClip?.overlay?.text&&<div className="customOverlay" style={{left:`${activeClip.overlay.x}%`,top:`${activeClip.overlay.y}%`,fontSize:`${activeClip.overlay.size}px`,fontWeight:activeClip.overlay.bold?800:500}}>{activeClip.overlay.text}</div>}<div className="stageInfo"><span>{activeClip?.title||'Preview'}</span><span>{ratio} • {duration}</span></div></div>:<div className="emptyCanvas"><div className="emptyCanvasContent"><span className="eyebrow">✦ AI Director ready</span><h1>Turn an idea into a scene.</h1><p>Describe the subject, mood, movement and outcome. Vidigen will shape the brief, generate the shot and place it on your timeline.</p><div className="miniHint">Start in the Director panel → Create production</div></div></div>}</div>
  ...
  <details className="advancedSettings">
    ...
    <div className="advancedBody">
      ...
      <label>AI model</label><select value={model} onChange={e=>setModel(e.target.value)}>{MODELS.map(([id,n])=><option key={id} value={id}>{n}</option>)}</select>
      {activeClip&&<button disabled={reframing} onClick={autoReframe}>{reframing?'Reframing…':`Auto-reframe selected clip to ${ratio}`}</button>}
    </div>
  </details>
  <div className="generationSummary"><div><span>Ready to generate</span><b>{mode} • {duration} • {ratio}</b></div><span className={online?'okText':'badText'}>{online?'● Provider ready':'● Connection needs attention'}</span></div>
