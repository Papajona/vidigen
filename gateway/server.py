...
class RenderClip(BaseModel):
    ...
    mediaType: str = Field(default='video', pattern=r'^(video|image)class RenderRequest(BaseModel):
    model_config=ConfigDict(extra='forbid')
    ratio: str = Field(default='16:9', pattern=r'^(16:9|9:16|1:1|4:5|21:9)$')
...
def _run_ffmpeg_render(inputs: list[tuple[str,int,int|None,str]], ratio: str, audio_file: str|None, output: str) -> dict:
    ...
    try:
        for idx,(src,start_ms,end_ms,media_type) in enumerate(inputs):
            ...
            dur=max(0.1,(end_ms-start_ms)/1000.0) if end_ms is not None else 5.0
            if media_type == 'image':
                # Turn a still image into a short video segment so images can be placed on
                # the same timeline and exported through the normal MP4 renderer.
                cmd=['ffmpeg','-y','-loop','1','-i',src,
                     '-f','lavfi','-i','anullsrc=channel_layout=stereo:sample_rate=48000',
                     '-t',str(dur),'-map','0:v:0','-map','1:a:0',
                     '-vf',_ratio_filter(ratio),'-c:v','libx264','-preset','veryfast','-crf','20',
                     '-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',part]
            else:
...
async def _bill_generation(user: dict | None, req: Generate) -> dict:
    ...
    # Only a true image-output request is billed as an image operation. Image → Video
    # contains the word "image" but still creates a video and must use video credits.
    operation = 'image' if mode_text in ('text → image','text -> image','text to image') else 'video'
