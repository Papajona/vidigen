...
def _provider_candidates(requested: str, req: Any) -> list[str]:
    ...
    required_source = {'image-to-video':'image','video-to-video':'video'}.get(capability)
    if required_source and not req.sourceUrl:
        raise HTTPException(400, f'{req.mode} requires a {required_source} source.')
    if required_source and req.sourceType and req.sourceType != required_source:
        raise HTTPException(400, f'{req.mode} requires a {required_source} source.')
...
class RenderOverlay(BaseModel):
    ...
    size: float = Field(default=42, ge=8, le=160)
    x: float = Field(default=50, ge=0, le=100)
    y: float = Field(default=82, ge=0, le=100)
    bold: bool = True

class RenderClip(BaseModel):
    model_config=ConfigDict(extra='forbid')
    uri: str = Field(min_length=8, max_length=4000)
    mediaType: str = Field(default='video', pattern=r'^(video|image)$')
    ...
    brightness: float = Field(default=100, ge=50, le=150)
    contrast: float = Field(default=100, ge=50, le=150)
    saturation: float = Field(default=100, ge=0, le=200)
    blur: float = Field(default=0, ge=0, le=12)
    rotation: float = Field(default=0, ge=-180, le=180)
    scale: float = Field(default=100, ge=50, le=150)
...
def _atempo_chain(speed: float) -> list[str]:
    speed = max(0.25, min(4.0, float(speed or 1)))
    filters=[]
    remaining=speed
    while remaining < 0.5:
...
def _run_ffmpeg_render(inputs: list[tuple[str,int,int|None,str,RenderClip]], ratio: str, audio_file: str|None, output: str) -> dict:
    import subprocess
    tmp_parts=[]
    try:
        for idx,(src,start_ms,end_ms,media_type,clip) in enumerate(inputs):
            part=str(Path(output).with_name(f'part-{idx}.mp4'))
            clip_filter=_clip_video_filter(ratio, clip)
            speed=float(clip.speed)
            if media_type == 'image':
                duration=max(0.1,(end_ms-start_ms)/1000.0) if end_ms is not None else 5.0
                duration=duration/max(0.25,min(4.0,speed))
                cmd=['ffmpeg','-y','-loop','1','-i',src,'-f','lavfi','-i','anullsrc=channel_layout=stereo:sample_rate=48000',
                     '-t',str(duration),'-map','0:v:0','-map','1:a:0','-vf',clip_filter,
                     '-c:v','libx264','-preset','veryfast','-crf','20','-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',part]
            else:
                ...
                cmd += [part]
            rr=subprocess.run(cmd,capture_output=True,text=True,timeout=RENDER_TIMEOUT_SECONDS)
            if rr.returncode != 0 or not Path(part).exists():
                raise RuntimeError(f'ffmpeg clip {idx+1} failed: {rr.stderr[-1200:]}')
        ...
        rr=subprocess.run(['ffmpeg','-y','-f','concat','-safe','0','-i',concat_file,'-c','copy',joined],capture_output=True,text=True,timeout=RENDER_TIMEOUT_SECONDS)
        if rr.returncode != 0 or not Path(joined).exists():
            raise RuntimeError(f'ffmpeg concat failed: {rr.stderr[-1200:]}')
        if audio_file:
        ...
        rr=subprocess.run(cmd,capture_output=True,text=True,timeout=RENDER_TIMEOUT_SECONDS)
        if rr.returncode != 0 or not Path(output).exists():
            raise RuntimeError(f'ffmpeg finalization failed: {rr.stderr[-1200:]}')
        ...
        if size>MAX_RENDER_BYTES: raise RuntimeError('Rendered master exceeds the configured output-size limit.')
...
async def render_project(req: RenderRequest, request: Request, user=Depends(auth)):
    ...
    with tempfile.TemporaryDirectory(prefix='vidigen-render-') as tmp:
        ...
        for i,clip in enumerate(req.clips):
            ...
            source_pairs.append((str(src),clip.trimStartMs,clip.trimEndMs,clip.mediaType,clip))
        audio_file=None
...
async def r2_presign(req: R2PresignRequest, request: Request, user=Depends(auth)):
    ...
    uid = user.get('sub') if isinstance(user, dict) else None
    if not uid:
        raise HTTPException(401, 'Signed-in user required for Cloudflare R2 uploads.')
...
@app.post('/api/r2-register')
async def r2_register(req: R2RegisterRequest, request: Request, user=Depends(auth)):
    """Register a completed R2 upload, record its actual size, and enforce the caller's storage plan."""
    uid = user.get('sub') if isinstance(user, dict) else None
    if not uid:
        raise HTTPException(401, 'Signed-in user required.')
    ...
    try:
        ...
        head = await asyncio.to_thread(s3.head_object, Bucket=bucket, Key=safe_key)
    except Exception:
        raise HTTPException(404, 'Uploaded R2 object was not found.')
    if head.get('ContentType') and not str(head.get('ContentType')).startswith(req.kind + '/'):
        raise HTTPException(415, 'Uploaded media type does not match the registered asset type.')
    existing = []
    if persistence.enabled():
        existing = await persistence.sb_request(
            'GET', 'assets',
            params={'user_id': f'eq.{uid}', 'storage_key': f'eq.{safe_key}', 'select': '*', 'limit': '1'}
        )
    if existing:
        row = existing[0]
        return {
            'ok': True,
            'asset_id': row.get('id'),
            'output_url': f'{cdn_base}/{safe_key}',
            'bytes': int(row.get('bytes') or 0),
            'existing': True,
        }
    actual_bytes = max(0, int(head.get('ContentLength') or 0))
    if persistence.enabled():
        used = await _storage_used_bytes(uid)
        limit = await _storage_limit_bytes(uid)
        if used + actual_bytes > limit:
            try:
                await asyncio.to_thread(s3.delete_object, Bucket=bucket, Key=safe_key)
            except Exception:
                log.exception('Storage quota exceeded but temporary R2 object could not be deleted')
            raise HTTPException(413, 'Storage limit reached. Remove existing media or upgrade your plan.')
        asset_id = await persistence.create_asset(
            uid, None, req.kind, safe_key, head.get('ContentType') or None, actual_bytes,
            {'name': req.name or '', 'source': 'user_upload'}
        )
    else:
        asset_id = None
    return {
...
async def _bill_generation(user: dict | None, req: Generate) -> dict:
    ...
    model=default_model if not req.model or req.model=='replicate' else req.model
    ...
    if provider_candidates:
        ...
        for attempt, provider_name in enumerate(provider_candidates):
            ...
            try:
                ...
                if next_provider: log.warning(f'Provider {provider_name} failed during generation submission: {e}. Failing over to {next_provider}.')
                else: log.error(f'All configured generation providers failed; last provider {provider_name}: {e}')
                continue
            except Exception as e:
...
async def r2_register(req: R2RegisterRequest, request: Request, user=Depends(auth)):
    ...
    if not uid:
        raise HTTPException(401, 'Signed-in user required.')
    endpoint, access_key, secret_key, bucket, cdn_base = (
        os.getenv(k, '') for k in ('R2_ENDPOINT','R2_ACCESS_KEY_ID','R2_SECRET_ACCESS_KEY','R2_BUCKET','R2_PUBLIC_BASE_URL')
    )
    if not all((endpoint, access_key, secret_key, bucket, cdn_base)):
        raise HTTPException(501, 'Cloudflare R2 is not fully configured on this gateway.')
    safe_key = _scoped_r2_key(uid, req.object_key)
    try:
        ...
        raise HTTPException(404, 'Uploaded R2 object was not found.')
    actual_type = str(head.get('ContentType') or '').lower()
    if actual_type and not actual_type.startswith(req.kind + '/'):
        raise HTTPException(415, 'Uploaded media type does not match the registered asset type.')

    existing = await persistence.sb_request(
        'GET','assets',params={'user_id':f'eq.{uid}','storage_key':f'eq.{safe_key}','select':'*','limit':'1'}
    ) if persistence.enabled() else []
    if existing:
        row=existing[0]
        return {'ok':True,'asset_id':row.get('id'),'output_url':f'{cdn_base.rstrip("/")}/{safe_key}',
                'bytes':int(row.get('bytes') or 0),'existing':True}
    ...
    if used + actual_bytes > limit:
        try:
            ...
            log.exception('Could not delete an upload rejected for exceeding storage quota')
        raise HTTPException(413, f'Storage limit reached. Remove existing media or upgrade your plan.')
    asset_id=await persistence.create_asset(
        uid,None,req.kind,safe_key,actual_type or None,actual_bytes,
        {'name':req.name or '','source':'user_upload'}
    ) if persistence.enabled() else None
    return {'ok':True,'asset_id':asset_id,'output_url':f'{cdn_base.rstrip("/")}/{safe_key}',
            'bytes':actual_bytes,'limit_bytes':limit,'used_bytes':used+actual_bytes}
...
async def storage_me(request: Request, user=Depends(auth)):
    ...
    used=await _storage_used_bytes(uid)
    limit=await _storage_limit_bytes(uid)
    return {'used_bytes':used,'limit_bytes':limit,'used_mb':round(used/1_000_000,2),
