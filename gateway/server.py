...
def _run_ffmpeg_render(inputs: list[tuple[str,int,int|None,str]], ratio: str, audio_file: str|None, output: str) -> dict:
    import subprocess
    tmp_parts=[]
    try:
        for idx,(src,start_ms,end_ms,media_type) in enumerate(inputs):
            part=str(Path(output).with_name(f'part-{idx}.mp4'))
            if media_type == 'image':
                duration=max(0.1,(end_ms-start_ms)/1000.0) if end_ms is not None else 5.0
                cmd=['ffmpeg','-y','-loop','1','-i',src,'-f','lavfi','-i','anullsrc=channel_layout=stereo:sample_rate=48000',
                     '-t',str(duration),'-map','0:v:0','-map','1:a:0','-vf',_ratio_filter(ratio),
                     '-c:v','libx264','-preset','veryfast','-crf','20','-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',part]
            else:
                probe_audio=subprocess.run(['ffprobe','-v','error','-select_streams','a:0','-show_entries','stream=index','-of','csv=p=0',src],capture_output=True,text=True,timeout=30)
                has_audio=bool(probe_audio.stdout.strip())
                ...
                if end_ms is not None:
                    cmd += ['-t',str(max(0.1,(end_ms-start_ms)/1000.0))]
                cmd += [part]
            rr=subprocess.run(cmd,capture_output=True,text=True,timeout=RENDER_TIMEOUT_SECONDS)
            if rr.returncode != 0 or not Path(part).exists():
                raise RuntimeError(f'ffmpeg clip {idx+1} failed: {rr.stderr[-1200:]}')
...
async def render_project(req: RenderRequest, request: Request, user=Depends(auth)):
    ...
    with tempfile.TemporaryDirectory(prefix='vidigen-render-') as tmp:
        ...
        for i,clip in enumerate(req.clips):
            src=Path(tmp)/f'source-{i}.bin'
            await _download_render_source(clip.uri,src)
            source_pairs.append((str(src),clip.trimStartMs,clip.trimEndMs,clip.mediaType))
        audio_file=None
        if req.backgroundAudioUri:
            ap=Path(tmp)/'audio.bin'; await _download_render_source(req.backgroundAudioUri,ap); audio_file=str(ap)
        out=Path(tmp)/'master.mp4'
