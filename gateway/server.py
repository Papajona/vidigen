...
class R2RegisterRequest(BaseModel):
    model_config = ConfigDict(extra='forbid')
    object_key: str = Field(min_length=1, max_length=500)
    kind: str = Field(pattern=r'^(image|video|audio)    model_config = ConfigDict(extra='forbid')
    media_url: str = Field(min_length=1, max_length=2000)
...
async def _storage_used_bytes(uid: str) -> int:
    if not persistence.enabled():
        return 0
    rows = await persistence.sb_request(
        'GET', 'assets',
        params={'user_id': f'eq.{uid}', 'select': 'bytes'}
    )
...
async def r2_register(req: R2RegisterRequest, request: Request, user=Depends(auth)):
    """Register a completed user upload, record its actual R2 size, and enforce the plan storage cap."""
    uid = user.get('sub') if isinstance(user, dict) else None
    if not uid:
        raise HTTPException(401, 'Signed-in user required.')
    ...
    if not all((endpoint, access_key, secret_key, bucket)):
        raise HTTPException(501, 'Cloudflare R2 is not configured on this gateway.')
    ...
    try:
        ...
        raise HTTPException(404, f'Uploaded R2 object was not found: {e}')
    ...
    if existing:
        row = existing[0]
        return {'ok': True, 'asset_id': row.get('id'), 'output_url': f"{os.getenv('R2_PUBLIC_BASE_URL','').rstrip('/')}/{safe_key}", 'bytes': int(row.get('bytes') or 0), 'existing': True}
    ...
    if used + actual_bytes > limit:
        ...
        raise HTTPException(413, f'Storage limit reached. This upload would use {actual_bytes} bytes and exceed your plan limit.')

    public_base = os.getenv('R2_PUBLIC_BASE_URL','').rstrip('/')
    asset_id = None
    if persistence.enabled():
        asset_id = await persistence.create_asset(
            uid, None, req.kind, safe_key, head.get('ContentType') or None,
            actual_bytes, {'name': req.name or '', 'source': 'user_upload'}
        )
    return {
        'ok': True,
        'asset_id': asset_id,
        'output_url': f'{public_base}/{safe_key}' if public_base else None,
        'bytes': actual_bytes,
        'limit_bytes': limit,
        'used_bytes': used + actual_bytes,
    }
